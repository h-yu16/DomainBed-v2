...
from domainbed.lib.misc import (
    random_pairs_of_minibatches, ParamDict, MovingAverage, l2_between_dicts, split_meta_train_test, accuracy_batch
)
from domainbed.lib.optimizer_helper import get_optim_and_scheduler
...
class ERM(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        self.optimizer, self.scheduler = get_optim_and_scheduler(self.network, hparams)
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        if self.hparams["optimizer"] == "Adahessian":
            loss.backward(create_graph=True)
        else:
            loss.backward()
        self.optimizer.step()

        acc, _, _ = accuracy_batch(self, all_x, all_y)

        return {'loss': loss.item(), "train_acc": acc}
...
class IRM(ERM):
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        return {'loss': loss.item(), 'nll': nll.item(),
            'penalty': penalty.item(), "train_acc": acc}
...
class Mixup(ERM):
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        for (xi, yi), (xj, yj) in random_pairs_of_minibatches(minibatches):
            ...
            index = torch.randperm(xi.shape[0]).to(device)
            x = lam * xi + (1 - lam) * xj[index]
            predictions = self.predict(x)

            objective += lam * F.cross_entropy(predictions, yi)
            objective += (1 - lam) * F.cross_entropy(predictions, yj[index])
        ...
        return {'loss': objective.item(), "train_acc": acc}
...
class MLDG(ERM):
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        return {'loss': objective, "train_acc": acc}
...
class AbstractMMD(ERM):
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        all_y = torch.cat([y for x,y in minibatches])
        acc, _, _ = accuracy_batch(self, all_x, all_y)
        return {'loss': objective.item(), 'penalty': penalty, "train_acc": acc}
...
class SagNet(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        self.optimizer_f, self.scheduler_f = get_optim_and_scheduler(self.network_f, hparams)
        self.optimizer_c, self.scheduler_c = get_optim_and_scheduler(self.network_c, hparams)
        self.optimizer_s, self.scheduler_s = get_optim_and_scheduler(self.network_s, hparams)
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        return {'loss_c': loss_c.item(), 'loss_s': loss_s.item(),
                'loss_adv': loss_adv.item(), 'train_acc': acc}
    ...
    def get_lr(self):
        if self.scheduler_f:
            lr_f = self.scheduler_f.get_last_lr()
            lr_c = self.scheduler_c.get_last_lr()
            lr_s = self.scheduler_s.get_last_lr()
            assert lr_f == lr_c and lr_c == lr_s
            return lr_f
        else:
            return [self.hparams["lr"],]
...
class Fishr(Algorithm):
    ...
    def _init_optimizer(self):
        self.optimizer, self.scheduler = get_optim_and_scheduler(self.network, self.hparams)
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        return {'loss': objective.item(), 'nll': all_nll.item(), 'penalty': penalty.item(), 'train_acc': acc}
